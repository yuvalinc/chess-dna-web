...
export default function Settings() {
  ...
  const { allGames, availableTimeClasses } = useChessData();
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="flex gap-1 mb-5 border-b border-chess-border/30 pb-2">
        {([
          { id: 'profile' as const, label: '\u{1F464} Profile' },
          ...(isAdmin ? [{ id: 'ai' as const, label: '\u{1F916} AI' }] : []),
          { id: 'settings' as const, label: '\u2699\uFE0F Settings' },
          ...(isAdmin ? [{ id: 'analytics' as const, label: '\u{1F4CA} Analytics' }] : []),
        ]).map((t) => (
          <button
            key={t.id}
        ...
        <>
          <Section title="Appearance">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <label className="text-sm text-gray-400 w-32">Theme</label>
              ...
              <div className="flex items-start gap-3">
                <label className="text-sm text-gray-400 w-32 pt-1">Board</label>
